...
import {
  AcmeReactiveSelection,
  dragOriginSelection,
  globalLastDisplayIndex,
...
export function List(props: ListProps) {
  ...
  const [displayList, setDisplayList] = createSignal<DisplayList>(
    fastList.signal[0](),
  );
  ...
  const unsubscribe = createEffect(
    on(
      [
        selection.globalIsDragging,
        selection.lastTouchedIndex,
        fastList.signal[0],
      ],
      () => {
  ...
  function handleDrop() {
    ...
    if (
      ...
      fastList?.moveItems(dragOriginSelection.keys, dragOriginList, [
        beforeId || null,
        afterId || null,
      ]);
      if (dragOriginSelection !== selection) {
  ...
  onMount(() => {
    keyboardShortcuts.registerHandler(
      "keydown",
      props.view.id,
      getListKeyboardHandler({
        fastList,
        selection,
        scrollRef,
      }),
      ...
      >
        ...
        >
          {displayList().length ? (
            ...
            <div
              class={styles["blank-list"]}
              onMouseOver={() => selection.setLastTouchedIndex(0)}
            >
