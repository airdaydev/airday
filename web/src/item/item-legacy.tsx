...
export function Item(props: ItemProps) {
  ...
  function getSelectClasses() {
    ...
    if (
      !props.selection.keys.has(props.displayList()[props.listIndex - 1]?.id)
    ) {
  ...
  const onClick = (event: MouseEvent) => {
    ...
    if (!event.shiftKey) {
      ...
      const mouseMove = (mouseUpEvent: MouseEvent) => {
        ...
        if (
          distance(origin, [mouseUpEvent.clientX, mouseUpEvent.clientY]) > 3
        ) {
    ...
    if (event.shiftKey && props.selection.keys.size) {
      ...
      const firstSelectedIndex = props.fastList.getFirstIndexOfSet(
        props.selection.keys,
      );
      if (firstSelectedIndex === false) return;
      if (props.listIndex < firstSelectedIndex) {
        const lastIndex = props.fastList.getLastIndexOfSet(
          props.selection.keys,
        );
        if (!lastIndex) return;
        ...
        const keys = props.fastList.getKeysInRange(
          firstSelectedIndex,
          props.listIndex,
        );
        props.selection.clear();
    ...
    <>
      {edit() && (
        ...
        >
          ...
          />
          <div ref={dummyRef} class={styles["dummy-ref"]} />
          <textarea
            ref={textAreaRef}
            value={item.text}
            onInput={(event) => {
      ...
      {!edit() && (
        ...
        >
          <div class={styles[`item-content-box`]}>
            ...
            <div>
              <div
                style={`display: flex; white-space: pre-line; max-width: 48em;`}
              >
                {props.item[0]().sticker && (
                  <Sticker set="baseline" item={props.item} />
                )}
                <span>{props.item[0]().text}</span>
              </div>
            </div>
          </div>
        </div>
