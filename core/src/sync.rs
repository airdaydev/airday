...
mod tests {
    use super::*;
    use crate::doc::{Doc, LIST_CURRENT};
    ...
    fn flush_in_idle_pushes_when_pending() {
        ...
        eng.doc_mut().add_item(LIST_CURRENT, "thing").unwrap();
        eng.flush();
    ...
    fn pushing_dirty_re_pushes_after_ack() {
        ...
        eng.doc_mut().add_item(LIST_CURRENT, "first").unwrap();
        eng.flush();
        let _first_push = eng.pop_outbox().expect("first PushOps");

        // Mutate again while the push is in flight.
        let item_id = eng.doc_mut().add_item(LIST_CURRENT, "during-push").unwrap();
        eng.flush();
    ...
    fn flush_during_pull_pushes_when_pull_completes() {
        ...
        eng.doc_mut().add_item(LIST_CURRENT, "during pull").unwrap();
        eng.flush();
    ...
    fn broadcast_during_idle_applies_and_acks() {
        ...
        let remote_blob = make_remote_blob(&dek, |d| {
            d.add_item(LIST_CURRENT, "from peer").unwrap();
        });
        ...
        let names: Vec<_> = eng
            .doc()
            .items_in_list(LIST_CURRENT, false)
            .into_iter()
            .map(|i| i.text)
            .collect();
    ...
    fn broadcast_during_pushing_does_not_clobber_in_flight() {
        ...
        eng.doc_mut()
            .add_item(LIST_CURRENT, "local-pushing")
            .unwrap();
        eng.flush();
        ...
        let remote_blob = make_remote_blob(&dek, |d| {
            d.add_item(LIST_CURRENT, "peer-during-push").unwrap();
        });
        eng.handle_server_bytes(&enc(&ServerFrame::OpsBroadcast {
            ops: vec![StoredOp {
                id: 5,
    ...
    fn multi_batch_pull_reaches_idle_only_on_complete() {
        ...
        let blob1 = make_remote_blob(&dek, |d| {
            d.add_item(LIST_CURRENT, "p1").unwrap();
        });
        eng.handle_server_bytes(&enc(&ServerFrame::OpsBatch {
        ...
        let blob2 = make_remote_blob(&dek, |d| {
            d.add_item(LIST_CURRENT, "p2").unwrap();
        });
        eng.handle_server_bytes(&enc(&ServerFrame::OpsBatch {
    ...
    fn disconnect_from_pushing_clears_in_flight() {
        ...
        drive_to_idle(&mut eng);
        let _ = drain_events(&mut eng);
        eng.doc_mut().add_item(LIST_CURRENT, "stranded").unwrap();
        eng.flush();
        let _ = eng.pop_outbox().unwrap();
        let _ = drain_events(&mut eng);
    ...
    fn end_to_end_two_engines_converge_via_engine_loop() {
        ...
        let item_a = a.doc_mut().add_item(LIST_CURRENT, "from-a").unwrap();
        a.flush();
