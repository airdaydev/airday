-- Shared local doc storage (spec/local-storage.md): an append-only
-- `ops` log keyed by (doc_id, local_seq) plus one `snapshots` row per
-- doc. Mirrors the sqlite schema the server uses. This is the generic,
-- client-agnostic portion of the schema; callers that need their own
-- tables (e.g. the CLI's `account` identity row) supply them as extra
-- migrations through `open_with_extra`, sharing the same db file and
-- `_migrations` ledger.
