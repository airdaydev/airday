...
1. `POST /api/account/prelogin { email }` → server returns `{ password_salt, kdf_params }`. (Or 404 if email unknown — but rate-limit either way to avoid email enumeration / online password guessing.)
...
3. POST `/api/account/recover { email, recovery_auth_secret }` → server verifies `SHA-256(recovery_auth_secret) == recovery_auth_hash`, returns `{ recovery_wrapped_dek, recovery_wrapped_dek_nonce, recovery_session_token }`. **Wrap is never released without this proof.** Endpoint is rate-limited.
...
- Email verification on signup: defer to SaaS-only (sprint 2+)?
- Rate limiting on `/prelogin`, `/login`, `/recover` — online guessing + email enumeration defense. Per-IP and per-email backoff. `/recover` is the most security-critical (gates the recovery wrap).
- Should `/prelogin` 404 on unknown email (leaks email existence) or always return a deterministic dummy salt (slows enumeration but wastes client Argon2 cycles)?
- `recovery_session_token` TTL — 15 min default? Should it survive the user typing in a long new password.
