...
- Threshold: when `(latest_op_id − latest_snapshot.up_to_op_id) > snapshot_threshold_ops` (default `10_000`, configurable via `snapshot_threshold_ops` in the server config or `AIRDAY_SNAPSHOT_THRESHOLD_OPS`), snapshotting becomes eligible for that account.
