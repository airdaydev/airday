...
import { DndContext, TreeContext, SolidListContext } from "./dnd-context";
...
export const Tree = (props: TreeComponentProps) => {
  ...
  onMount(() => {
    ...
    treeContext.scrollContainerRef = scrollContainerRef;
  });
  ...
  const showBackdropPlaceholder = () => {
    const show =
      treeContext.isDraggingOver() &&
      treeContext.lastTouchedIndexSignal[0]() ===
        signal().window.length + signal().start &&
      !treeContext.isDragOrigin;
    return show;
  };

  createEffect(
    on(
      () => [treeContext.isDraggingOver(), treeContext.dndContext.isDragging()],
      (val) => {
        if (val[0] && val[1]) {
          autoscroller.start();
        } else {
  ...
  const signal = treeContext.getWindowedSignal(containerVector);
  return (
    <div
      classList={{
        focus: treeContext.isFocused(), // TODO: Do better
      ...
      style={{
        ...
        "z-index": 2,
        "overflow-y":
          treeContext.dndContext.dragMode[0]() === "touch"
            ? "hidden"
      ...
      onScroll={(event) => {
        ...
        if (
          ...
          treeContext.itemHeight * 10
        ) {
          scrollSignal[1](event.target.scrollTop);
        }
      }}
      onFocus={() => treeContext.setFocus()}
      onMouseLeave={() => treeContext.leave()}
      tabIndex={-1}
    >
      <div
        style={`position: relative;
              ...
              width: 100%;
              z-index:10;
              min-height: ${treeContext.presentCount() * treeContext.itemHeight}px;`}
      >
      ...
      <div
        ...
        style={{
          ...
          ...(props.hideBackdrop && {
            bottom: 0,
            // More hacks...
            "z-index": treeContext.dndContext.isDragging() ? 20 : 0,
            position: "absolute",
            "min-height": `${treeContext.itemHeight}px`,
          }),
        }}
        onMouseDown={() => {
          treeContext.clearSelection();
          treeContext.setFocus();
        }}
        onMouseEnter={() => {
          if (treeContext.dndContext.isDragging()) {
            treeContext.setDragOver();
            const endIndex = signal().window.length + signal().start;
            if (treeContext.isDragOrigin) {
              treeContext.setLastTouchedIndex(endIndex - 1);
              treeContext.setDragOver();
            } else {
              // foreign entry
