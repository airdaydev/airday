...
function renderCache(canvas, ctx2D) {
...
export class EventWorker {
  ...
  offscreenScale() {
    ...
    this.canvas.width = transform.dayPx * transform.scale;
    this.canvas.height = transform.hourPx * 25 * transform.scale;
    this.ctx2D.scale(transform.scale, transform.scale);
  ...
  onMessage = (message: MessageEvent) => {
    if (message.data.type === "resize") {
      ...
      this.offscreenScale();
      renderCache(this.canvas, this.ctx2D);
    }
