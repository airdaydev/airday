// Digit shortcuts for the fixed nav views: 1 Focus, 2 Inbox, 3 Done,
// 4 Bin. Pure decision logic, extracted from Workspace so it can be
// unit-tested without a DOM. The Bin slot mirrors its nav visibility —
// it only exists while the bin holds items, so 4 is a no-op when empty.
