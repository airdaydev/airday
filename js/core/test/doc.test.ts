...
describe("Doc + MemStorage", () => {
  test("addItem → save → put → get → load → fingerprint matches", async () => {
    ...
    const itemId = doc.addItem(LIST_CURRENT, "buy milk");
    ...
    const items = JSON.parse(restored.itemsInListJson(LIST_CURRENT, false));
  ...
  test("seeded built-in lists are present after create", () => {
    const doc = Doc.create();
    const lists = JSON.parse(doc.allListsJson()) as Array<{ id: string }>;
    const ids = lists.map((l) => l.id);
    expect(ids).toContain(LIST_CURRENT);
    expect(ids).toContain(LIST_HOLDING);
  });
});
...
describe("Op stream round trip via two replicas", () => {
  test("replica B converges to A's fingerprint after applying A's blob", () => {
    ...
    const itemId = a.addItem(LIST_CURRENT, "from A");
    ...
    const items = JSON.parse(b.itemsInListJson(LIST_CURRENT, false));
